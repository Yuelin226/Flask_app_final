from flask import Flask, render_template, request, session, send_file, Response, redirect, url_for
...
app.config['SECRET_KEY'] = "Yuelin says this is secret key and no one should access it"
...
def create_figure():
    ...
    neg=uploaded_df_sentiment[uploaded_df_sentiment['Sentiment']=='Negative']
    neg_review=neg['Review Text']
    
    #pos_review=pos['Review Text']
    
    app_str='\n'.join(textReview['Review Text'])
    #Just the negative words
    sent = app_str
    pos_word_list = []
    neg_word_list = []
    neu_word_list = []
    ...
    wordcloud = WordCloud(stopwords = stopwords,width=1600,height=800 ,colormap='Set1', collocations=False,mask=mask,background_color="white").generate(''.join(neg_review)) #neg_word_list
